...
function parseDatabaseUrl(url) {
    ...
    if (!url.startsWith('postgresql://')) {
        throw new Error('Backup script currently only supports PostgreSQL databases (postgresql://). Local SQLite files should be backed up by copying the .db file directly.');
    }
    ...
    if (!match) {
        throw new Error('Invalid PostgreSQL DATABASE_URL format. Expected: postgresql://user:pass@host:port/db');
    }
...
async function createBackup() {
    try {
        ...
        // Parse URL (will throw if not PostgreSQL)
        ...
        // Don't kill process on dev/test failures intentionally unless critical
        process.exit(1);
    }
}
